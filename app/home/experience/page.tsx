        ...
        <div className="flex justify-evenly items-center h-full">
            <div className="flex flex-col gap-6">
                {companies?.map((company) => (
                    <div className="flex">
                        <CompanyCard key={company.id} id={company.id} company={company.name} position={company.position} image_url={company.image_url}/>
                    </div>
                ))}
            </div>
            {searchParams?.companyId &&
              companyById.map((company) => (
                <div className="w-[25vw] h-[80vh] bg-black rounded-lg">
                  <Image
